...
public class VideoPanel extends JPanel {
	...
	synchronized public void paint(Graphics g) {
		...
		if (image != null) {
			 ...
			 * nav.flyToPortal(portal, image, true); distance =
			 * DistanceMeaure.getDistanceToObject(image.getHeight(), (portal.a *
			 * 2 + portal.b * 2) / 2, Config.portalHeight, 1.6404040404040405);
	...
	synchronized private Mat findAndDrawEllipse(Mat sourceImg) {
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			if (contours.get(i).toArray().length > 5) {
				rotatedrect = Imgproc.fitEllipse(new MatOfPoint2f(contours.get(i).toArray()));
double aspect = rotatedrect.boundingRect().height / rotatedrect.boundingRect().width;
				if (aspect > 0.9 && aspect < 1.8 && rotatedrect.boundingRect().area() > 20000 &&rotatedrect.boundingRect().area()<300000) {
					...
					if (rotatedrectbest == null) {
						...
						contour2fbest = new MatOfPoint2f(contours.get(i).toArray());
